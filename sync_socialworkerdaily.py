...
from build_bank_v4 import YEARS, SUBJECTS, fetch, html_text, parse_questions, social_url

DATA = Path(__file__).parent / 'data' / 'bank.json'


def should_check(data):
    last = data.get('meta', {}).get('socialworkerdaily_last_checked_at')
    if not last:
        return True
    try:
...
def main():
    ...
    if not should_check(data):
        print(json.dumps({'status': 'skip', 'reason': '距離上次成功檢查未滿5天'}, ensure_ascii=False))
        return
